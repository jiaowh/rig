...
> on its own merits. Also measured: conformal coverage at solver-SELECTED recipes was 1/9 miss
...
| — WP-E remaining (dependency-ordered): (1) SCBO/constrained-EI + TuRBO BoTorch slate + powered M2 re-run with the multi-obj/constrained comparators (qLogNEHVI Phase-II acquisition now DONE, slice 5); (2) normalizing-flow typicality + PGD δ-box for §8; (3) DKL / SNGP-single deploy path. | TODO | slices 2-5 DONE |
